...
void heapSort(struct TreeNode *node) {
    ...
    for (i = MAXSIZE - 1; i >= 1; i--) {
        swap(&node[i], &node[0]);
        // exchange the largest element into its location
        // at decreasing space in node
        Max_heapify(node, 0, --size);
...
void buildHeap(struct TreeNode *node) {
    int i;
    for (i = (MAXSIZE / 2); i >= 0; i--) Max_heapify(node, i, MAXSIZE);
}
...
void Max_heapify(struct TreeNode *node, int i, int size) {
    ...
    if (r < size && node[r].data > node[largest].data) largest = r;

    if (largest != i) {
        int temp;
        // if array i is not the largest, then swap it with the largest child
        // exchange node[i] with node[largest] the exchange may have violated the
        // heap propery.
        temp               = node[i].data;
        node[i].data       = node[largest].data;
        node[largest].data = temp;

        Max_heapify(node, largest,
                    size);  // reconstruct the heap property recursively
    }
